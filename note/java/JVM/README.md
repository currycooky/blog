...
通过`-Xss=N`来改变线程的栈大小。如果设置的过小，当线程的调用栈非常大时，会抛出`StackOverflowError`。

#### 偏向锁

锁偏向于对它访问最为频繁的线程。

如果一个线程最近用到了某个锁，那么线程下一次执行由同一把锁保护的代码所需的数据，可能仍然保存在处理器的缓存中，如果给这个线程优先获取这把锁的权利，缓存的命中率可能就会增加，性能会有所提高。

而对于不同的线程有同等机会访问争用的锁，可以考虑使用`-XX:-UseBiasedLocking`

禁用偏向锁，会稍稍改进性能。

#### 自旋锁

在处理同步锁的竞争问题时，JVM有两种选择。第一种是让线程进入忙循环，执行一些指令，然后再次检查这个锁；另一种是把线程放到队列中，在锁可用时通知它。

如果锁被持有的时间很短，那么第一种方式就会比第二种快得多。反之则第二种更快。JVM会在这两种情况间寻求合理的平衡，自动调整将线程提交到通知队列之前的自旋时间。

应该尽量使同步块尽可能的短，这样可以影响JVM处理自旋锁的方式，降低线程进入通知队列的机会。