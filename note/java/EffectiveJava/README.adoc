...
If you find yourslef having to write a `readObject` or `writeObject` method on a class that is not extendable by its clients, consider the serialization proxy pattern.